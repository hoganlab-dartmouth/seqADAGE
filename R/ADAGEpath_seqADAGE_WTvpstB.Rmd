...
```{r}
#seq_model <- read.csv('seqADAGE_model_e75_relu.csv', stringsAsFactors = F)
seq_model <- read.csv('seqADAGE_model_e75_sig.csv', stringsAsFactors = F)
...
```{r}
seq_model <- read.csv('../outputs/e_models/300s/pao1_aligned_rnaseq_compendium_zp2_MRnorm_log_01_bygene_seed:660_kl1:1e-10_kl2:0_act:sigmoid_init:glorot_uniform_ep:250_tied:True_batch:10_lr:1.0_en_weights_da.csv', stringsAsFactors = F, header=F)
...
#rnaseq_data_sums <- read.csv('MOPS_COCUL_WT.csv', stringsAsFactors = F, header = F)
#rnaseq_data_sums <- read.csv('COCUL_WT_phoB.csv', stringsAsFactors = F, header = F)
rnaseq_data_sums <- read.csv('PAO1_anr_salmon_reads.csv', stringsAsFactors = F, header = T)
rnaseq_data_sums$Name <- sapply(rnaseq_data_sums$Name, function(x) cDNA_to_PAO1(x))
#rnaseq_data_sums$geneID <- sapply(rnaseq_data_sums$geneID, function(x) name)
colnames(rnaseq_data_sums) <- c('geneID','WT_1','WT_2','anr_1','anr_2')
```
...
rnaseq_data_normed_seq <- zeroone_norm(input_data = rnaseq_data_raw_seq_temp, 
                                       use_ref = TRUE,
                                       ref_data = seq_compendium)
...
```{r}
rnaseq_data_activity_seq <- calculate_activity(input_data = rnaseq_data_normed_seq, 
                                               model = seq_model,
                                               HW_cutoff = 2.5)
```

```{r}
data_pheno <- c(rep("WT",2),rep('anr',2))
```
...
active_sigs_seq <- get_active_signatures(limma_result = limma_result_seq,
                                     pheno_group = "both", 
                                     method = "pareto", N_fronts = 15)
```

```{r}
plot_weight_dists(seq_model,'seq_eADAGE')
```

```{r}
plot_raw_data(rnaseq_data_raw, rnaseq_data_raw_seq_temp, 'seq_eADAGE')
pdf(paste('seq_eADAGE','_volcano_test.pdf',sep=''), height=4,width=4)
plot_volcano(limma_result_seq, highlight_signatures = active_sigs,
             interactive = F)
dev.off()

pdf(paste('seq_eADAGE','_DA_heatmap.pdf',sep=''), height=6,width=6)
...
signature_similarity <- plot_signature_overlap(selected_signatures = active_sigs_seq,
                                               model = seq_model)
```
...
```{r}
visualize_gene_network(selected_signatures = unique_active_sigs_seq,
                       gene_color_value = gene_logFC,
